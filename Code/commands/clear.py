...
def clear_command(message):
    ...
    for student in students:
        keyboard.add(
            types.InlineKeyboardButton(student.name, callback_data=f"clear_student:{student.id}")
        )
    bot.send_message(message.chat.id, "Which student?", reply_markup=keyboard)


@bot.callback_query_handler(func=lambda call: call.data.startswith("clear_student:"))
def clear_pick_student(call):
    ...
    storage.clear_stickers(student_id)
    bot.send_message(chat_id, f"{student.name}'s sticker count is now 0.")
